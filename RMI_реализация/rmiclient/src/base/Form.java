...
import operation.HostelOperation;
...
public class Form extends javax.swing.JFrame {
    ...
    static HostelOperation hostelOperation = null;
    
    static DefaultTableModel modelS = new DefaultTableModel();
    static ServiceOperation serviceOperation = null;
    ...
    try {
            txtTotalSumma.setText(Integer.toString(hostelOperation.getSumOfHostel()));
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
      ...
      {model.removeRow(tbl.getSelectedRow());
          ...
          JOptionPane.showMessageDialog(this,"Table is Empty.");
            }else{JOptionPane.showMessageDialog(this,"Please select Single Row For Delet.");
            }
        }
    }//GEN-LAST:event_btnDeleteActionPerformed
    private void doVivod(List<Hostel> lstHostel){
        doClearTable();
        int i = 1;
        for(Hostel hostel: lstHostel){
    ...
    private void doClearTable(){
     ...
     private void doVivodS(List<Service> lstService){
        doClearTableS();
        int i = 1;
    ...
    private void btnAddHostelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddHostelActionPerformed
        ...
        try {
            doVivod(hostelOperation.addNewHostel(el));
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private void btnDecideSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDecideSActionPerformed
        try {
            txtTotalSummaS.setText(Integer.toString(serviceOperation.getSumOfService()));
        } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Не удалось установить соединение с сервером:" + ex.getMessage() + ".",
                    "Ошибка",
    ...
    private void btnAddServiceActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddServiceActionPerformed
       dlgAddServ.setVisible(false);
        ...
        try {
            doVivodS(serviceOperation.addNewService(el));
        } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Не удалось добавить. Попытайтесь повторить попытку :" + ex.getMessage() + ".",
                    "Ошибка",
    ...
    private void BookingWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_BookingWindowOpened
        try {
            hostelOperation = (HostelOperation) Naming.lookup("//localhost:1199/rmiTest");
    ...
    private void ServicesWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_ServicesWindowOpened
           try {
            serviceOperation = (ServiceOperation) Naming.lookup("//localhost:1199/rmiTest1");
            modelS = (DefaultTableModel) tblS.getModel();
            doVivodS(serviceOperation.getListOfService());
        } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Не удалось установить соединение с сервером:" + ex.getMessage() + ".",
                    "Ошибка",
